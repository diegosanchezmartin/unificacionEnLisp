...
(defun sustituir (listaComposicion valorAcomprobar)
  (unless (null listaComposicion)
    (setq valor (first (last listaComposicion)))
    (write valor)
    (format t "~%")
    (write (first valorAcomprobar))
    (format t "~%")
    (if (equalp valor (first valorAcomprobar))
      (if (eq (atom (first listaComposicion)) 'T)
        (first listaComposicion)
        (first (first listaComposicion))
      )
